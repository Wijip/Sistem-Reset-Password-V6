...
const Dashboard: React.FC<DashboardProps> = ({ requests, personnel, currentUser, siteSettings }) => {
  ...
  const stats = useMemo(() => {
    ...
    const active = relevantRequests.filter(r => r.status !== RequestStatus.SELESAI).length;
    ...
    >
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className={`text-3xl font-black tracking-tight mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Dashboard {isSuperAdmin ? 'Super Admin' : 'Admin'}</h1>
